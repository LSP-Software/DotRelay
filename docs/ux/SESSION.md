...
e2e suite as the verification surface).

Scope:
- Phase 5 of campaign #231: the in-browser Recovery & Security area in
  the workspace shell, replacing the CLI-side Recovery Kit entry.

Changed:
- Boundary account-key envelope passthrough, so the browser opens the
  account-key objects it verifies.
- `apps/web/lib/account-keys.ts` client module: setup, unlock by recovery
  code / encryption password / passkey PRF, rotation, password add/remove,
  transfer send and redeem, and the offline retry state.
- `apps/web/app/workspace/workspace-shell.tsx`: the Recovery area renders
  the state matrix (sign-in prompt, device-setup prompt, one-time code
  setup, unlocked status and actions, offline retry card).
- Prebuilt argon2 worker IIFE at `apps/web/public/argon2-worker.js`
  (generated by `apps/web/scripts/build-argon2-worker.ts`; the dev script
  regenerates it; the artifact is checked in so `next build` and e2e need
  no build step); the shell fetches it at init and the client's
  `runArgon2id` falls back to the main thread without it.
- `apps/web/playwright.config.ts` honours `E2E_WEB_PORT` so the suite can
  run against a dedicated fixture instance.
- `apps/web/e2e/workspace-recovery.spec.ts` (new): six scenarios - the
  one-time code display as an `alertdialog`, uniform wrong-secret failure,
  rotation retiring the old code, encryption-password add/use-after-reload/
  remove, a transfer redeemed across two browsers (including the
  expired-transfer conflict copy), and the offline retry card; plus the
  phone-viewport hold.
- Docs: `docs/web-application.md`, `docs/wiki/trust.md`,
  `docs/wiki/persistence.md`, `docs/wiki/architecture.md`,
  `docs/ux/JOURNEYS.md`, `docs/ux/DECISIONS.md` (D-004 - D-006), and a
  stale comment in `apps/web/e2e/workspace-enrollment-storage.spec.ts`.

Verified:
- `bun run check` green.
- Full Playwright e2e suite green on the dedicated fixture port 8300
  (`E2E_WEB_PORT`); `workspace-recovery.spec.ts` 6/6.
- `biome ci` clean.

Remaining:
- No e2e coverage for passkey-PRF unlock: the Playwright virtual
  authenticator exposes no PRF, so that path stays unit-covered.
- Phase 6 of #231 is the maintainability pass that follows.
